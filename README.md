...
**¿Por qué Angular con standalone components y signals?**
Es el enfoque moderno de Angular: menos *boilerplate* (sin NgModules) y estado
reactivo con `signal`/`computed`, que actualiza la UI sola cuando cambian los datos.

**¿Por qué rutas relativas + proxy en vez de poner la URL del backend?**
Para que todo sea **mismo origen**: evita CORS y permite compartir la app por un túnel
con un solo link, sin cambiar configuración. La URL del backend no queda "quemada" en
el código.

**¿Por qué un `BaseApiService` genérico?**
Para no repetir el CRUD en cada entidad. Cada servicio solo declara su recurso y hereda
`listar/obtener/crear/actualizar/eliminar`. Menos código y más consistente.

**¿Dónde se guarda la sesión y el carrito? ¿Es seguro?**
En `localStorage` + `signals`. Es cómodo y sobrevive recargas. Para la **seguridad real**
mandan las validaciones del backend; los guards del front son de navegación/UX.

**¿Cómo separan tienda y administración?**
Por **layouts** y **rutas**: la tienda cuelga de `/` con `StorefrontLayout`, y el panel
de `/admin` con `AdminLayout` protegido por `adminGuard` (rol ADMIN).

**¿Los pagos funcionan de verdad?**
El flujo está completo y el `Pago` se registra en la base, pero la **pasarela está
simulada**. Dejamos marcado el punto exacto para integrar Mercado Pago real.

**¿Cómo manejan las imágenes?**
Se cargan desde el form del admin, se convierten a **base64** y se guardan en el campo
`imagen` del producto; se muestran en toda la tienda con respaldo a un degradado.

**¿Por qué el QR no usa una librería?**
Para no sumar dependencias (ni reinstalar/reiniciar). `utils/qrcode.ts` genera el SVG
del QR de forma autocontenida.
